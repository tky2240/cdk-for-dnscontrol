...
class DnscontrolDomain extends constructs_1.Construct {
  ...
  constructor(scope, id, props) {
    ...
    for (const providerProps of props.providerPropsList) {
      new dnscontrol_domain_provider_1.DnscontrolDomainProvider(
        this,
  ...
  _getDomainConfig(node, domainConfig) {
    if (
      dnscontrol_domain_provider_1.DnscontrolDomainProvider.isDnscontrolDomainProvider(
        node,
      )
    ) {
    ...
    if (
      dnscontrol_raw_record_1.DnscontrolRawRecord.isDnscontrolRawRecord(node)
    ) {
